...
class RectUpsamplerTemporal(nn.Module):
    def __init__(self,
                 grid_size_lo: int,
                 grid_size_hi: int,
                 n_features: int = 4,
        ...
        self.height, self.width = self.grid_hi, self.grid_hi # CAREFUL: THIS ASSUMES SQUARE GRIDS
        ...
        self.register_buffer("neighbor_indices", self._compute_neighbors_adjusted()) # COMPUTE INSTEAD ON HR SCALE

        # Learnable weights for upsampling (C, F, P_hi, k)
        self.weight_map = nn.Parameter(
            torch.zeros(num_classes, n_features, self.p_hi, self.k, 2)
        )
    ...
    def forward(self, x: torch.Tensor, cls_ids: torch.Tensor = None, y_prev = None) -> torch.Tensor:
        """
        ...
        # reshape y_prev if given
        if y_prev is not None:
            y_prev = y_prev.view(BS, F, self.grid_hi, self.grid_hi).view(BS, F, -1)  # (BS, F, P_hi)
        ...
        for f in range(F):
            # concatenate y_previous time step if given
            if y_prev is not None:
                y_low_db_concat = torch.stack([y_low_db[:, f], y_prev[:, f]], dim=-1)  # (BS, P_hi)
            
            neigh = y_low_db_concat[:, idx]               # (BS, P_hi, k, 2) # CHECK? ???
            weights = self.weight_map[cls_ids, f]        # (BS, P_hi, k, 2)
            out[:, f] = (neigh * weights).sum(dim=(-2,-1))        # (BS, P_hi)
...
class LocalResiduals(nn.Module):
    ...
    def forward(self, y_in, cls_ids = None, return_latent=False):
        ...
        if self.softmax:
            w = Func.softmax(self.weight_map, dim=1)  # (npix, k, map_dim)
        else:
            w = self.weight_map
...
class RectUpsampleWithResidualsTemporal(nn.Module):
    def __init__(self,
                 grid_size_lo,
                 grid_size_hi,
                 n_features=4,
        ...
        super().__init__()
        self.upsampler = RectUpsamplerTemporal(
        ...
        if double_linear:
            ...
            raise NotImplementedError("double_linear=True requires LocalResiduals2, which is not implemented in this file.")
        else:
    ...
    def forward(self, x: torch.Tensor, cls_ids: torch.Tensor = None, 
                ...
                y_prev = None, eps: torch.Tensor = None) -> torch.Tensor:
        """
        Args:
            x         : (B, n_features * p_lo)
            cls_ids  : (B)
        ...
        """
        if y_prev is None:
            raise ValueError("y_prev must be provided for RectUpsampleWithResidualsTemporal")
        if return_latent:
            y_upsampled = self.upsampler(x, cls_ids, y_prev=y_prev)                # (B, F, H, W)
            ...
            y_upsampled = self.upsampler(x, cls_ids=cls_ids, y_prev=y_prev)                # (B, F, H, W)
            if self.split_residuals:
    ...
    @torch.no_grad()
    def sample(self, x: torch.Tensor, cls_ids: torch.Tensor = None, 
               y_prev = None,
