    ...
    def __init__(self, in_dim, out_dim, num_layer=2, hidden_dim=100, 
        ...
        if self.preproc:
            if self.input_dims_for_preproc is not None:
                ...
                raise NotImplementedError("Need to provide input_dims_for_preproc if preproc_layer is True.")
...
class HierarchicalWrapper(nn.Module):
    ...
    def _apply_remaining_models(self, out, eps=None, x_onehot=None, cls_ids=None, start_idx=1, return_intermediates=False):
    ...
    def forward(self, x, eps=None, x_onehot=None, cls_ids=None, return_intermediates=False):
        ...
        return self._apply_remaining_models(x, eps=eps, x_onehot=x_onehot,
    ...
    def forward_temporal(self, x, start_xc, x_onehot=None, cls_ids=None):
        ...
        T = x.size(0)
        time_series = torch.empty((T, start_xc.size(-1)), device=x.device)

        for t in range(T):
            if t == 0:
                inp = torch.cat([x[t].unsqueeze(0), start_xc.unsqueeze(0)], dim=1) # forward with BS = 1, dim (1, spatial_dim)
            else:
        ...
        return self._apply_remaining_models(time_series, eps=None, x_onehot=x_onehot,
                                            cls_ids=cls_ids, start_idx=1, return_intermediates=False)
        # TO DO: double check if forward has the correct shape and if forward and sample have the same shape
...
class ModelSpec:
    def __init__(self, model, vars_as_channels=False, use_one_hot=False, noise_dim=None, one_hot_option="concat",
                 temporal=False, prev_timestep_option="concat"):
        self.model = model
        self.vars_as_channels = vars_as_channels
        self.use_one_hot = use_one_hot
        self.noise_dim = noise_dim  # None = no noise, -1 = pass all remaining noise
        ...
        self.prev_timestep_option = prev_timestep_option

class HierarchicalWrapper(nn.Module):
    def __init__(self, model_specs, n_vars=1, one_hot_dim=0, model_specs_temporal=None):
        super().__init__()
        self.model_specs = model_specs  # list of ModelSpec
        self.model_specs_temporal = model_specs_temporal
        self.n_vars = n_vars
        self.one_hot_dim = one_hot_dim
        
    def _forward_temporal_onestep(self, x, start_xc, model_index=0, cls_ids=None, eps_input=None):
        # Temporal forward for one of the models
        spec = self.model_specs_temporal[model_index]
        model = spec.model
        T = x.size(0)
        time_series = torch.empty((T, start_xc.size(-1)), device=x.device)

        if spec.prev_timestep_option == "concat":
            for t in range(T):
                if t == 0:
                    inp = torch.cat([x[t].unsqueeze(0), start_xc.unsqueeze(0)], dim=1) # forward with BS = 1, dim (1, spatial_dim)
                else:
                ...
                if spec.use_one_hot and spec.one_hot_option == "argument" and cls_ids is not None:
                    time_series[t] = model(inp, cls_ids=cls_ids[t], eps=eps_input)[0]
                else:
                    time_series[t] = model(inp, eps=eps_input)[0]

        elif spec.prev_timestep_option == "argument":
            for t in range(T):
                inp = x[t].unsqueeze(0)
                if t == 0:
                    y_prev=start_xc.unsqueeze(0)
                else:
                    y_prev = time_series[t-1].unsqueeze(0)
                
                if spec.use_one_hot and spec.one_hot_option == "argument" and cls_ids is not None:
                    time_series[t] = model(inp, cls_ids=cls_ids[t], y_prev=y_prev, eps=eps_input)[0]
                else:
                    time_series[t] = model(inp, y_prev=y_prev, eps=eps_input)[0]
                    
        return time_series        
        
    def _forward_marginal_onestep(self, x, model_index=0, cls_ids=None, eps_input=None):
        # Marginal forward for one of the models
        spec = self.model_specs[model_index]
        model = spec.model

        if spec.use_one_hot and spec.one_hot_option == "argument" and cls_ids is not None:
            out = model(x, cls_ids=cls_ids, eps=eps_input)
        else:
            out = model(x, eps=eps_input)
        
        return out
    
    def _forward_temporal_onestep_norollout(self, x, y_prev, model_index=0, cls_ids=None, eps_input=None):
        spec = self.model_specs_temporal[model_index]
        model = spec.model
                
        if spec.prev_timestep_option == "concat":
            inp = torch.cat([x, y_prev], dim=-1) # Q: which dim??)
            if spec.use_one_hot and spec.one_hot_option == "argument" and cls_ids is not None:
                out = model(inp, cls_ids=cls_ids, eps=eps_input)
            else:
                out = model(inp, eps=eps_input)

        elif spec.prev_timestep_option == "argument":
            inp = x
            if spec.use_one_hot and spec.one_hot_option == "argument" and cls_ids is not None:
                out = model(inp, cls_ids=cls_ids, y_prev=y_prev, eps=eps_input)
            else:
                out = model(inp, y_prev=y_prev, eps=eps_input)
                    
        return out
    
    def _apply_remaining_models(self, out, eps=None, x_onehot=None, 
                                cls_ids=None, start_idx=1, return_intermediates=False, start_xc=None,
                                rollout=True, y_prev=None, temporal=True):
        """Pass the output through models[start_idx:], shared by forward and forward_temporal."""
        noise_cursor = 0
        intermediates = []
        specs = self.model_specs
        specs_temporal = self.model_specs_temporal if self.model_specs_temporal is not None else []
        # for i, spec in enumerate(specs[start_idx:]):
        for i in range(start_idx, len(specs)):
            spec = specs[i]
            
            if i < len(specs_temporal):
                spec_temporal = specs_temporal[i]
            else:
                spec_temporal = None
            # Open Q: in the following, use spec or spec_temporal?
            ...
            # Forward pass
            if i < len(specs_temporal) and temporal:  
                if rollout:
                    if start_xc is not None:
                        current_input = out
                        indices = cls_ids
                        k = len(specs) - 1 - i
                        start_xc_step = torch.nn.functional.avg_pool2d(start_xc.unsqueeze(0).view(1, -1, 128, 128), 
                                                                 kernel_size=2**k, stride=2**k).view(1, -1)
                        out = self._forward_temporal_onestep(current_input, start_xc=start_xc_step.squeeze(0), model_index=i, 
                                                        cls_ids=indices, eps_input=eps_input)               
                                            
                    else:
                        # one marginal forward pass with current model to get the start_xc for temporal forward
                        start_xc_step = self._forward_marginal_onestep(out[0].unsqueeze(0), model_index=i, 
                                                                cls_ids=cls_ids[0], eps_input=eps_input[:1]) 
                        current_input = out[1:]
                        indices = cls_ids[1:]
                        out = self._forward_temporal_onestep(current_input, start_xc=start_xc_step.squeeze(0), model_index=i, 
                                                        cls_ids=indices, eps_input=eps_input[1:])
                    
                        out = torch.cat([start_xc_step, out], dim = 0)
                elif y_prev is not None:
                    # no rollout, use yprev as argument
                    # average pool to match correct resolution
                    k = len(specs) - 1 - i
                    y_prev_step = torch.nn.functional.avg_pool2d(y_prev.view(y_prev.shape[0], -1, 128, 128), 
                                                                 kernel_size=2**k, stride=2**k).view(y_prev.shape[0], -1)
                    out = self._forward_temporal_onestep_norollout(out, y_prev_step, model_index=i, 
                                                                   cls_ids=cls_ids, eps_input=eps_input)
                else:
                    raise ValueError("Without rollout, need to specify yprev.")
            else:
                out = self._forward_marginal_onestep(out, model_index=i, cls_ids=cls_ids, eps_input=eps_input)
    ...
    def forward(self, x, eps=None, x_onehot=None, cls_ids=None, 
                return_intermediates=False, rollout=True, y_prev=None, start_xc=None, temporal=True):
        # Just reuse the helper for all models
        return self._apply_remaining_models(x, eps=eps, x_onehot=x_onehot,
                                            cls_ids=cls_ids, start_idx=0, return_intermediates=return_intermediates,
                                            rollout=rollout, y_prev=y_prev, start_xc=start_xc, temporal=temporal)


    """
    # do I need this? 
    # maybe just a special case of the above
    def forward_temporal(self, x, start_xc, x_onehot=None, cls_ids=None):
        #Temporal generation for the first model in model_specs.
        #After that, apply the remaining models as in forward().
    ...
    def sample_temporal(self, x, sample_size=10, expand_dim=True, start_xc=None, **kwargs):
        #Naive temporal sampling: just stack forward_temporal passes.
        outputs = [self.forward(x, start_xc=start_xc, **kwargs) for _ in range(sample_size)]
        if expand_dim:
            return torch.stack(outputs, dim=-1)
        else:
            return torch.cat(outputs, dim=-1)
    ...
    def sample(self, x, sample_size=10, expand_dim=True, **kwargs):
        #Naive temporal sampling: just stack forward_temporal passes.
        outputs = [self.forward(x, temporal=False,**kwargs) for _ in range(sample_size)]
        if expand_dim:
            return torch.stack(outputs, dim=-1)
        else:
            return torch.cat(outputs, dim=-1)
    ...
    def sample_temporal(self, x, sample_size=10, expand_dim=True, start_xc=None, **kwargs):
        #Naive temporal sampling: just stack forward_temporal passes.
        if start_xc is not None:
            assert start_xc.shape[-1] == sample_size
            outputs = [self.forward(x,  start_xc = start_xc[..., j], **kwargs) for j in range(sample_size)]
        else:
            outputs = [self.forward(x, **kwargs) for _ in range(sample_size)]
        if expand_dim:
            return torch.stack(outputs, dim=-1)
        else:
            return torch.cat(outputs, dim=-1)
