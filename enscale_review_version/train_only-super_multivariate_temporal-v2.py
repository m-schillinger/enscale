...
def visual_sample(model, x, y, save_dir, norm_method=None, norm_stats=None, sqrt_transform=True, square_data=False, mode_unnorm = "hr", 
                  one_hot_dim=None, logit=False, normal=False, fft=False, one_hot_in_super=False, conv=False, x_one_hot=None, y_prev=None):
    if not args.dropout:
        model.eval()
    with torch.no_grad():
        ...
        if args.latent_dim == len(args.variables) and args.nicolai_layers and args.add_intermediate_loss:
            ...
            if args.double_linear:
                gen, _, y_interm = model(x, return_latent=True, y_prev=y_prev)
                gen2, _, y_interm2 = model(x, return_latent=True, y_prev=y_prev)
            else:
                gen, y_interm = model(x, return_latent=True, y_prev=y_prev)
                gen2, y_interm2 = model(x, return_latent=True, y_prev=y_prev)
            ...
            gen, y_upsampled = model(x, cls_ids=cls_ids, return_mean=True, y_prev=y_prev)
            gen2, y_upsampled2 = model(x, cls_ids=cls_ids, return_mean=True, y_prev=y_prev)
            y_interm = gen - y_upsampled.view(gen.shape[0], -1)
            y_interm2 = gen2 - y_upsampled2.view(gen.shape[0], -1)
            plot_intermediate = True
        elif args.nicolai_layers:
            gen, y_upsampled = model(x, return_mean=True, y_prev=y_prev)
            gen2, y_upsampled2 = model(x, return_mean=True, y_prev=y_prev)
            y_interm = gen - y_upsampled.view(gen.shape[0], -1)
            y_interm2 = gen2 - y_upsampled2.view(gen.shape[0], -1)
            plot_intermediate = True
        else:
...
def get_eval_samples(current_model, current_test_loader, mode_unnorm="hr", norm_method=None, norm_stats=None, 
                     input_mode = "x", output_mode = "y", logit=False, normal=False, one_hot_in_super=False, conv=False, one_hot_dim=None, temporal=False):
    if not args.dropout:
        current_model.eval()
    samples = []
    trues = []
            ...
            if input_mode == "x":
                ...
                if not args.nicolai_layers:
                    ...
                    cls_ids = get_run_index_from_onehot(x_te[:, -one_hot_dim:], gcm_dict=gcm_dict, rcm_dict=rcm_dict, rcm_list=rcm_list, gcm_list=gcm_list,
                                                        mode="joint" if not args.ignore_one_hot_gcm else "rcm")
                    gen = current_model.sample(xc_te, cls_ids=cls_ids, sample_size=5, y_prev=y_te_prev)
                else:
                    gen = current_model.sample(xc_te, sample_size=5, y_prev=y_te_prev)
...
if __name__ == '__main__':
    ...
    if args.nicolai_layers:
        if args.server == "ada":
            ...
            save_dir = f"/cluster/work/math/climate-downscaling/cordex-data/cordex-ALPS-allyear/eng-results/super_temporal/{subfolder}/var-{variables_str}/loc-specific-layers_norm-out-{args.norm_method_output}_{args.save_name}/"
    else:
        raise NotImplementedError("Temporal training only implemented for nicolai layers.")
    ...
    if args.method == 'eng_2step':
        ...
        if args.one_hot_in_super and not args.ignore_one_hot_gcm:
            one_hot_dim = 7
            interm_dim += one_hot_dim
        elif args.one_hot_in_super and args.ignore_one_hot_gcm:
            one_hot_dim = 4
            interm_dim += one_hot_dim
        else:
            one_hot_dim = 0
        ...
        if args.add_x_in_super:
            raise NotImplementedError("removed this option for simplicity.")
        ...
        if args.nicolai_layers:
            ...
            model = RectUpsampleWithResidualsTemporal(128//args.kernel_size_lr, 
    ...
    for epoch_idx in range(args.resume_epoch, args.num_epochs):
        ...
        for batch_idx, data_batch in enumerate(current_loader):
            ...
            if args.nicolai_layers and not args.double_linear and args.add_intermediate_loss:
                gen1, y_interm = model(xc, cls_ids=cls_ids, return_latent=True, y_prev=y_prev)
                ...
                gen1, _, y_interm = model(xc, cls_ids=cls_ids, return_latent=True, y_prev=y_prev)
                gen2, _, y_interm2 = model(xc, cls_ids=cls_ids, return_latent=True, y_prev=y_prev)
            elif args.nicolai_layers and args.add_mse_loss:
                gen1, y_upsampled = model(xc, cls_ids=cls_ids, return_mean=True, y_prev=y_prev)
                gen2, y_upsampled2 = model(xc, cls_ids=cls_ids, return_mean=True, y_prev=y_prev)
            elif args.nicolai_layers:
            ...
            if epoch_idx == 0 or ((epoch_idx + 1) % (args.print_every_nepoch * 10) == 0):             
                ...
                with torch.no_grad():
                    for data_te in current_test_loader:
                        ...
                        if args.nicolai_layers and not args.double_linear and args.add_intermediate_loss:
                            gen1, y_interm = model(xc_te, cls_ids=cls_ids, return_latent=True, y_prev=y_te_prev)
                            gen2, y_interm2 = model(xc_te, cls_ids=cls_ids, return_latent=True, y_prev=y_te_prev)
                        elif args.nicolai_layers and args.double_linear and args.add_intermediate_loss:
                            gen1, _, y_interm = model(xc_te, cls_ids=cls_ids, return_latent=True, y_prev=y_te_prev)
                            gen2, _, y_interm2 = model(xc_te, cls_ids=cls_ids, return_latent=True, y_prev=y_te_prev)
                        elif args.nicolai_layers:
                            gen1 = model(xc_te, cls_ids=cls_ids, y_prev=y_te_prev)
                            gen2 = model(xc_te, cls_ids=cls_ids, y_prev=y_te_prev)
                        else:
