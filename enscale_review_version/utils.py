...
def count_parameters_wrapper(model, trainable_only: bool = True, temporal: bool = False) -> int:
    total_params = 0
    if not temporal:
        specs = model.model_specs
    else:
        specs = model.model_specs_temporal
    for spec in specs:
...
def get_run_index_from_onehot(
    ...
    if mode == "gcm":
        gcm_idx = one_hot_vec.argmax(dim=1).cpu().numpy()
        return gcm_idx

    if mode == "rcm":
        rcm_idx = one_hot_vec.argmax(dim=1).cpu().numpy()
        return rcm_idx
    ...
    # Faster reverse lookup (avoid repeated list.index calls)
    ...
    run_indices = [
        get_run_index(r, g, rcm_list, gcm_list, root)
        for r, g in zip(rcm_names, gcm_names)
    ]
