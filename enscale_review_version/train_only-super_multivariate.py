...
def get_eval_samples(current_model, current_test_loader, mode_unnorm="hr", norm_method=None, norm_stats=None, 
    ...
    with torch.no_grad():
        ...
        for data_te in current_test_loader:
            ...
            ndays = BS // 4
            if input_mode == "x":
                gen = current_model.sample(x_te[:ndays], sample_size=5)
            elif input_mode == "xc":                    
                if not args.nicolai_layers:
                    xc_te = add_one_hot(xc_te[:ndays], one_hot_in_super=one_hot_in_super, conv=conv, x=x_te, one_hot_dim=one_hot_dim)
                    gen = current_model.sample(xc_te[:ndays], sample_size=5)
                elif one_hot_in_super:
                    cls_ids = get_run_index_from_onehot(x_te[:ndays, -one_hot_dim:], gcm_dict=gcm_dict, rcm_dict=rcm_dict, rcm_list=rcm_list, gcm_list=gcm_list,
                                                        mode="joint" if not args.ignore_one_hot_gcm else "rcm")
                    gen = current_model.sample(xc_te[:ndays], cls_ids=cls_ids, sample_size=5)
                else:
                    gen = current_model.sample(xc_te[:ndays], sample_size=5)
            ...
            if args.nicolai_layers:
                if args.one_hot_in_super:
                    cls_ids = get_run_index_from_onehot(x[:, -one_hot_dim:], gcm_dict=gcm_dict, rcm_dict=rcm_dict, rcm_list=rcm_list, gcm_list=gcm_list,
                                                        mode="joint" if not args.ignore_one_hot_gcm else "rcm")
                    cls_ids = torch.tensor(cls_ids, device = device)
                    #x[:, -one_hot_dim:]
                else:
                    cls_ids = None
